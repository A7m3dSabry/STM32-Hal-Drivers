...
Std_ReturnType HAL_RCC_Init_Default(void) {
...
RCC_PeripheralClock_t HAL_RCC_UpdateSystemClockValues(void) {
    ...
    ret.CPU = systemFrequency /  (uint32_t)pow(2,temp);
    ...
    ret.APB1 = ret.CPU / (uint32_t)pow(2 ,temp);   // RCC_PPRE1_DIV_BY_1 = 3
    ...
    ret.APB2 = ret.CPU / (uint32_t)pow(2 , temp);   // RCC_PPRE2_DIV_BY_1 = 3
