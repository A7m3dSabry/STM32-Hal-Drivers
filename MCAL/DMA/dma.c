...
Std_ReturnType
HAL_DMA_RegisterInterruptCallback(const DMA_ChannelNumber_t channel, const InterruptCallback_t callback) {
...
void DMA1_Channel6_IRQHandler(void) {
    ...
    if (interruptsCallbacks[5] != NULL) {
        interruptsCallbacks[5](INTERRUPTS_NO_PARAMS);
    }
}
...
void DMA1_Channel7_IRQHandler(void) {
    ...
    if (interruptsCallbacks[6] != NULL) {
        interruptsCallbacks[6](INTERRUPTS_NO_PARAMS);
    }
}
